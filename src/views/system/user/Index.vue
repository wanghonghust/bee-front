<template>
  <el-row>
    ...
    <el-button
      ...
      @click="handleEdit(0, selectedUsers[0])"
    >
      编辑
    </el-button>
    <el-button
    ...
    <el-col>
      ...
      >
        ...
        <el-table-column label="所属角色" prop="role">
          <template #default="scope">
            <el-tag
              v-for="item in scope.row.role.map((item:any)=>item.name)"
              class="ml-2"
              type="success"
              >{{ item }}
            </el-tag>
        ...
        <el-table-column label="限流">
          <template #default="scope">
            <el-popover
              placement="right"
              :width="80"
              trigger="click"
              @hide="changeLimit(limitFormRef, scope.row)"
            >
              <template v-if="scope.row.limiter" #reference>
                <el-button
                  :type="scope.row.limiter.on ? 'danger' : 'info'"
                  size="small"
                  @click="showLimit(scope.row)"
                  circle
                >
                  {{ scope.row.limiter.on ? "ON" : "OFF" }}
                </el-button>
              </template>
              <template #default>
                <el-form
                ...
                >
                  ...
                  <el-form-item prop="limiter.limit">
                    <el-input
                      v-model.number="limitForm.limiter.limit"
                      type="text"
                      placeholder="最大限流次数"
                      :disabled="!limitForm.limiter.on"
                    />
                  </el-form-item>
                </el-form>
              </template>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            ...
            <el-avatar
              ...
              :src="
                scope.row.avatar
                  ? baseUrl + `system/file?id=` + scope.row.avatar
                  : ''
              "
              @click="editAvatar(scope.$index, scope.row)"
            >
              <template #default>
                <el-icon>
                  <plus></plus>
  ...
  >
    ...
    <el-button type="danger" @click="cancelUpload" style="margin-left: 2rem"
      >取消上传</el-button
    >
    <el-button type="primary" @click="uploadAvatar" :disabled="imageUrl === ''"
      >上传头像</el-button
    >
  </el-dialog>
</template>
...
const validateLimit = (rule: any, value: string, callback: any) => {
  if (!limitForm.limiter.on) {
    callback();
    return;
  }
  if (value === "") {
  ...
  if (!isNumber) {
    ...
    callback(new Error("限制次数大于等于1000！"));
  } else {
    callback();
  }
};
...
const limitRules = reactive({
  "limiter.limit": [{ validator: validateLimit, trigger: "blur" }],
});
...
interface LimterSource {
  on: boolean;
  limit: number;
}

const Limiter = reactive({
  on: false,
  limit: 1000,
});
...
let limitSource = reactive<LimterSource>({
  on: false,
  limit: 0,
});
...
const handleEdit = (index: number, row: any) => {
  ...
  editForm.role = row.role.map((item: { id: any }) => item.id);
...
const changeState = (index: number, row: any) => {
  ...
  ElMessageBox.confirm(dialogMsg.value, "警告", {
...
const deleteUsers = (users: UserInfo[]) => {
  ...
  ElMessageBox.confirm(`确认删除用户"${names}"？`, "警告", {
    distinguishCancelAndClose: true,
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    type: "warning",
...
const checkUsername = (rule: any, value: string, callback: any) => {
  ...
  if (!valid) {
    callback(
      new Error("用户名需以英文开始，由纯英文或英文和数字及下划线组成！")
    );
  } else {
    callback();
  }
...
const showLimit = (row: any) => {
  resetValidator(limitFormRef.value);
  limitForm.id = row.id;
  limitForm.limiter.on = row.limiter.on;
  limitForm.limiter.limit = row.limiter.limit;
  limitSource = JSON.parse(JSON.stringify(limitForm));
};
const changeLimit = async (formEl: FormInstance | undefined, row: any) => {
  if (!formEl) return;
  await formEl.validate((valid, fields) => {
    if (valid) {
      // 未修改，不提交
      if (JSON.stringify(limitForm) === JSON.stringify(limitSource)) {
        row.limiter = limitSource;
        return;
      }
      changeLimiter(limitForm)
        .then((res) => {
          row.limiter = limitForm.limiter;
          ElMessage.success("修改限流成功！");
        })
        .catch((err) => {
          ElMessage.error("修改限流失败！");
          row.limiter = limitSource;
        });
    }
  });
...
const resetValidator = (formEl: FormInstance | undefined) => {
  formEl?.resetFields();
};
onMounted(() => {
  getUsers();
